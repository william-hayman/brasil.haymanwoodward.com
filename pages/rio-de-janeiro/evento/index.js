...
export default function Home() {
  ...
  const url = "rio-de-janeiro/evento"
  ...
  return (
    <>
      ...
      <Cta  url={url} localeId={t('localeRJ')} eventDate={t('dateRJ')} eventType={t('live')} eventHour={eventHour}/>
      <Countdown deadline="2023-07-25 08:00" url={url}/>
      <About url={url}/>
      <Cases url={url}/>
      <Sponsors />
      <Questions />
...
Home.getLayout = function getLayout(page) {
  const { props } = page
  return (
      <Layout url="rio-de-janeiro/evento">{page}</Layout>
  )
}